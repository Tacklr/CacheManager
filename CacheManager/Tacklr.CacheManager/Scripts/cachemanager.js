...
(function (window, $, ko, toastr, Prism, undefined) {
    ...
    ko.templates['CacheTreeTemplate'] =
        '<!-- ko if: !$data.isEmpty() -->' +
            '<ul>' +
            '<!-- ko foreach: CM.Sort(CM.ObjectAsArray($data.Children), CM.SortCacheKey) -->' +
            '<li>' +
            ...
            '</li>' +
            '<!-- /ko -->' +
            '<!-- ko foreach: CM.Sort($data.Values, CM.SortCacheKey) -->' +
            '<li>' +
            ...
            '<span data-bind="text: Text, attr: { title: Key }"></span>' +
            '</li>' +
            '<!-- /ko -->' +
            '</ul>' +
            '<!-- /ko -->';
    ko.templates['EntryDetailsTemplate'] =
        '<div class="modal-header" tabindex="-1">' +
            '<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="fa fa-times fa-fw"></span></button>' +
            ...
            '<button type="button" class="btn btn-danger pull-left" data-bind="click: CM.DeleteNode(Key)">Delete</button>' +
    ...
    Modal.Generate = function (template, options) {
        ...
        var $modalContainer = $('<div id="modal-container" class="modal ' + (opts.Animate ? 'fade' : '') + '" tabindex="-1" role="dialog" aria-hidden="true" aria-labelledby="modal-title">' +
    ...
    var getVerificationToken = function (url, op_timeout) {
        ...
        var delay = op_timeout || 30 * 1000; //default? (30 seconds right now)
        var src = url + (url.indexOf('?') > -1 ? "&" : "?") + "_=" + nonce();
        var dfd = $.Deferred();
        if ($.active++ === 0) {
            $.event.trigger('ajaxStart');
        }
        var $iframe = $('<iframe src="' + src + '" style="height: 0; width: 0; border: 0; padding: 0; margin: 0; position: absolute; top: 0; left: 0;">') //pass token name? callback name? better hiding?
            .on('load', function () {
